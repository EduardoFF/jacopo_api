...
#ifndef __USE_DEBUG_NAV
#define __USE_DEBUG_NAV 0
#endif

#ifndef __USE_ERROR_NAV
#define __USE_ERROR_NAV 0
#endif
...
RVONavClient::RVONavClient(UInt8 robot_id, CCI_Robot &robot) :
  ...
  m_targetMinOrientationAngle(20.0),
...
RVONavClient::init(TConfigurationNode& t_node) 
{
  ...
  if (NodeExists(t_node, "targetMinOrientationAngle")) 
  {
    GetNodeText(GetNode(t_node, "targetMinOrientationAngle"), text);
    sscanf(text.c_str(), "%f", &m_targetMinOrientationAngle);
    /// default for pop distance, double
  }
...
RVONavClient::update() 
{
      ...
      (m_mobMode == MOBILITY_LCM || m_mobMode == MOBILITY_MANUAL || m_mobMode == MOBILITY_LCM_POP)) 
  {
    ...
    Real offset = (m_targetPosition - position).Length();
    if (offset < m_targetMinPointDistance) 
      {
	DEBUGNAV("Reached destination position with offset %.2f\n",
		 offset);
	if( m_doTargetOrientation )
	  {
	    DEBUGNAV("Doing Orientarion\n");	    
	    ...
	    if( fabs(cdev)  < m_targetMinOrientationAngle )
	      {
		setState( STATE_ARRIVED_AT_TARGET );
	      }
	    else
...
void
RVONavClient::setTargetOrientation(CDegrees ori)
{
  CRadians oA;
  CVector3 axis;
  DEBUGNAV("Set TargetOrientation %.2f\n", ori.GetValue());
  if( m_enforcing )
    {
  ...
  m_targetOrientation = ori.GetValue();
}
...
RVONavClient::initLocalNavigation(TConfigurationNode& t_node) 
{
  ...
  if (NodeExists(t_node, "local_navigation")) 
  {
    ...
    GetNodeAttributeOrDefault(node, "targetOrientation", 
			      m_targetOrientation, 
			      m_targetOrientation);
    DEBUGNAV("doTargetOrientation? %d\n", m_doTargetOrientation);
